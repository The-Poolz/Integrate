...
contract("Flex Staking with LockedDealV2 integration", accounts => {
    ...
    it('should return reward tokens', async () => {
        ...
        assert.equal(BigNumber.sum(oldBal, reward).toString(), actualBal.toString(), 'invalid balance')
    })
    ...
    it('should return locked tokens', async () => {
        ...
        assert.equal(parseInt(locked).toString(), result.toString(), 'invalid locked amount')
        assert.equal(BigNumber.sum(oldBal, result).toString(), actualBal.toString(), 'invalid balance')
    })
    ...
    it('should mass stake', async () => {
        ...
        for (let i = 0, rwdId = 2, lockId = 3; i < users.length; i++, rwdId += 2, lockId += 2) {
            const reward = ((amounts[i] * APR * durations[i]) / 365 / 24 / 60 / 60) / 100
            await lockToken.transfer(users[i], amounts[i])
            await lockToken.approve(flexStaking.address, amounts[i], { from: users[i] })
            await timeMachine.advanceBlockAndSetTime(startTime)
            await flexStaking.Stake(poolId, amounts[i], durations[i], { from: users[i] })
            await timeMachine.advanceBlockAndSetTime(finishTime)
    ...
    it('should withdraw tokens', async () => {
        ...
        finishTime = Math.floor(date.getTime() / 1000) + 60
        await timeMachine.advanceBlockAndSetTime(finishTime)
        const rewardReserve = new BigNumber(await rwdToken.balanceOf(flexStaking.address))
        ...
        const actualReserv = new BigNumber(await rwdToken.balanceOf(flexStaking.address))
        assert.equal(actualReserv, '0', 'invalid reserve amount')
        assert.equal(actualBal.toString(), BigNumber.sum(rewardReserve, ownerBal).toString(), 'invalid owner balance')
    })
