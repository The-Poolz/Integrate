...
contract(
  "Integration Between Envelop Token, WhiteList and LockedDeal",
  (accounts) => {
    let token,
      originalToken,
      lockedDealContract,
    ...
    describe("setting up Envelop Token", () => {
      it("should deploy new envelop token", async () => {
        ...
        const whitelistSettings = await whitelistContract.WhitelistSettings(
          whitelistId
        );
        ...
        assert.equal(
          ...
          cap.multipliedBy(10 ** 18).toString(),
          "check first address balance"
        );
        assert.equal(
          lockedAddress,
          lockedDealContract.address,
          "check lockedDeal address"
        );
        assert.equal(
          whitelistAddress,
          whitelistContract.address,
    ...
    describe("Integration with LP contract", () => {
      ...
      it("should transfer when in whitelist", async () => {
        ...
        let result = await whitelistContract.Check(
          whitelistAddress,
          whitelistId
        );
      ...
      it("should revert when not in whitelist", async () => {
        ...
        const swapAmount = "1000";
        const result = await whitelistContract.Check(
          notWhitelistAddress,
          whitelistId
        );
        ...
        assert.equal(
          balance.toString(),
          swapAmount,
          "check notWhitelistAddress balance"
        );
      });
      ...
      it("check when the finish time is over", async () => {
        ...
        await newToken.SetLockingDetails(
          testToken2.address,
          timestamps,
          ratios,
          past,
    ...
    describe("Integration with Locked Deal and Whitelist Contract", () => {
      ...
      const runSimulation = (amountToActivate, blockTime) => {
        ...
        for (let i = 0; i < timestamps.length; i++) {
          const amount = amountToActivate
            .multipliedBy(ratios[i])
            .dividedToIntegerBy(ratios.length);
        ...
        if (totalAmount.isLessThan(amountToActivate)) {
          ...
          if (unlockAmounts.length) {
            unlockAmounts[unlockAmounts.length - 1] =
              unlockAmounts[unlockAmounts.length - 1].plus(difference);
          } else {
      ...
      it("activating token before all unlocks", async () => {
        ...
        assert.equal(
          userOriginalBalance.toString(),
          sim.creditableAmount.toString()
        );
        assert.equal(
          tx.logs[tx.logs.length - 1].args.Amount.toString(),
          sim.totalAmount.toString()
        );
        const promises = [];
        ...
        dealData.forEach((item, index) => {
          assert.equal(
            item.unlockTime.toString(),
            sim.unlockTimes[index].toString()
          );
          assert.equal(
            item.amount.toString(),
            sim.unlockAmounts[index].toString()
          );
