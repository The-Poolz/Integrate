...
contract("Integration between PoolzBack, Uniswap and Benefit", (accounts) => {
  ...
  describe("Setting up PoolzBack", () => {
    ...
    it("should set address of Benefit contract", async () => {
      await poolzBack.SetBenefit_Address(benefit.address, {
        from: firstAddress,
      });
    ...
    it("should set address of whiteList contract", async () => {
      await poolzBack.SetWhiteList_Address(whiteList.address, {
        from: firstAddress,
      });
    ...
    it("Creating new WhiteList for pool", async () => {
      const tx = await createNewWhiteList(
        whiteList,
        poolzBack.address,
        firstAddress
      );
    ...
    it("should create new ETH Pool", async () => {
      await testToken.approve(poolzBack.address, amount, {
        from: firstAddress,
      });
      const date = new Date();
  ...
  describe("Setting up Benefit Contract", () => {
    ...
    it("Create liquidity pair", async () => {
      const result = await uniswapV2Factory.createPair(
        pozToken.address,
        testToken.address,
        { from: firstAddress }
      );
  ...
  describe("Benefit and LP integration", () => {
    ...
    it("calc tokens with LP", async () => {
      ...
      await pozToken.transfer(uniswapV2Pair.address, amount.toString(), {
        from: pozHolder,
      });
      await testToken.transfer(uniswapV2Pair.address, amount.toString(), {
        from: pozHolder,
      });
      ...
      assert.equal(
        amount,
        balance,
        "checking how many tokens were added to LP"
      );
      const result = await benefit.CalcTotal(pozHolder);
    ...
    it("check when remove lp traking", async () => {
      ...
      assert.equal(
        expected.toString(),
        actual.toString(),
        "calc tokens without LP contract"
      );
    });
  ...
  describe("Investing in Pool", () => {
    ...
    it("Fail to invest when investor has no POZ", async () => {
      ...
      await truffleAssert.reverts(
        poolzBack.InvestETH(poolId, {
          from: investor,
          value: web3.utils.toWei("0.4"),
        }),
    ...
    it("should invest in Pool when it is open for all", async () => {
      ...
      const future = Math.floor(date.getTime() / 1000) + 60;
      await timeMachine.advanceBlockAndSetTime(future);
      await poolzBack.InvestETH(poolId, {
        from: investor,
        value: web3.utils.toWei("0.4"),
      });
      const tokenDecimals = await testToken.decimals();
      const realRate = publicRate.div(
        new BN(10).pow(new BN(21 + tokenDecimals.toNumber() - 18))
      ); // converting decimal21Rate to realRate
