...
contract("Interation Between PoolzBack and WhiteList for Investing", (accounts) => {
    ...
    describe("Creating Whitelist of Investors and New Pool", () => {
        ...
        describe("New ERC20 Pool", () => {
            ...
            it("should create new pool with ERC20 as Main Coin", async () => {
                ...
                const d21PozRate = ercPozRate.mul(
                    new BN("10").pow(new BN(21 + tokenDecimals.toNumber() - mainCoinDecimals.toNumber()))
                )
                const d21PublicRate = ercPublicRate.mul(
                    new BN("10").pow(new BN(21 + tokenDecimals.toNumber() - mainCoinDecimals.toNumber()))
                )
                const tx = await poolzBack.CreatePool(
                    ercTestToken.address,
                    future,
                    d21PublicRate,
                    d21PozRate,
                    startAmount,
    ...
    describe("Investing in ETH Pool", () => {
        ...
        it("should Fail to invest when investor not whitelisted", async () => {
            ...
            await truffleAssert.reverts(
                poolzBack.InvestETH(ethPoolId, { from: fakeInvestor, value: web3.utils.toWei("0.1") }),
    ...
    describe("Investing in ERC20 Pool", () => {
        const shouldInvest = async (index) => {
            ...
            const tokenDecimals = await ercTestToken.decimals()
            const mainCoinDecimals = await mainCoin.decimals()
            const tokens = new BigNumber(ercAllowance[index])
                .div(new BigNumber(10).pow(mainCoinDecimals.toString()))
                .multipliedBy(ercPozRate.toString())
        ...
        it("should Fail to invest when investor not whitelisted", async () => {
            ...
            await truffleAssert.reverts(
                poolzBack.InvestERC20(ercPoolId, fakeInvestorAmt, { from: fakeInvestor }),
                "Sorry, no alocation for Subject"
            )
        })
    })
    ...
    describe("should get info", async () => {
        ...
        it("should return OutOfstock", async () => {
            ...
            const result = await poolzBack.CreatePool(
                ercTestToken.address,
                future,
                d21PublicRate,
                d21PozRate,
