...
contract("MultiWithdraw", (accounts) => {
    ...
    it("should create mass pools", async () => {
        ...
        for (let i = 0; i < owners.length; i++) startTimeStamps.push(future)
        finishTime = future = future + 60 * 60 * 24 * 30
        const finishTimeStamps = []
        for (let i = 0; i < owners.length; i++) finishTimeStamps.push(future)
        const startAmounts = [allow, allow, allow, allow, allow]
        const tx = await lockedDealV2.CreateMassPools(
            token.address,
            startTimeStamps,
            finishTimeStamps,
            startAmounts,
    ...
    it("should mass withdraw", async () => {
        ...
        let poolIds = []
        for (let i = 0; i <= lastPoolId; i++) poolIds.push(i.toString())
        let oldBalances = []
