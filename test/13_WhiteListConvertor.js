...
contract("Integration Between PoolzBack and WhiteListConvertor", (accounts) => {
    ...
    it("should create white list", async () => {
        ...
        await whiteList.CreateManualWhiteList(timestamp, whiteListConvertor.address, {
            from: firstAddress,
            value: whiteListCost
        })
    ...
    describe("should invoke only by creator", async () => {
        ...
        it("Only creator can change creator", async () => {
            await truffleAssert.reverts(whiteList.ChangeCreator(whiteListId, secondAddress), "Only creator can access")
        })

        it("Only creator can change contract", async () => {
            await truffleAssert.reverts(whiteList.ChangeContract(whiteListId, secondAddress), "Only creator can access")
        })
    ...
    describe("should fail", async () => {
        ...
        it("should fail with incorrect number of users", async () => {
            await truffleAssert.reverts(
                whiteList.AddAddress(whiteListId, [firstAddress], ["100000000", "100"], { from: accounts[5] }),
